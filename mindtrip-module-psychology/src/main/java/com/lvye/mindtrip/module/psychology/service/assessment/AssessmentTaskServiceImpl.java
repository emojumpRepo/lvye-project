...
import com.lvye.mindtrip.framework.common.util.date.DateUtils;
import com.lvye.mindtrip.framework.common.util.number.NumberUtils;
...
import com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.*;
import com.lvye.mindtrip.module.psychology.controller.admin.profile.vo.StudentAssessmentQuestionnaireDetailVO;
import com.lvye.mindtrip.module.psychology.controller.admin.profile.vo.StudentAssessmentTaskDetailVO;
import com.lvye.mindtrip.module.psychology.controller.admin.profile.vo.StudentAssessmentTaskHisVO;
...
import com.lvye.mindtrip.module.psychology.dal.dataobject.assessment.AssessmentUserTaskDO;
...
import com.lvye.mindtrip.module.psychology.dal.mysql.assessment.AssessmentDeptTaskMapper;
import com.lvye.mindtrip.module.psychology.dal.mysql.assessment.AssessmentResultMapper;
import com.lvye.mindtrip.module.psychology.dal.mysql.questionnaire.QuestionnaireResultMapper;
import com.lvye.mindtrip.module.psychology.dal.mysql.assessment.AssessmentTaskQuestionnaireMapper;
import com.lvye.mindtrip.module.psychology.dal.mysql.assessment.AssessmentTaskMapper;
import com.lvye.mindtrip.module.psychology.dal.mysql.assessment.AssessmentUserTaskMapper;
import com.lvye.mindtrip.module.psychology.enums.AssessmentTaskStatusEnum;
import com.lvye.mindtrip.module.psychology.enums.ErrorCodeConstants;
import com.lvye.mindtrip.module.psychology.enums.ParticipantCompletionStatusEnum;
import com.lvye.mindtrip.module.psychology.enums.RiskLevelEnum;
import com.lvye.mindtrip.module.psychology.util.RiskLevelUtils;
...
import com.lvye.mindtrip.module.system.api.permission.PermissionApi;
...
// 危机事件相关
import com.lvye.mindtrip.module.psychology.dal.dataobject.consultation.CrisisInterventionDO;
import com.lvye.mindtrip.module.psychology.dal.dataobject.consultation.CrisisEventProcessDO;
import com.lvye.mindtrip.module.psychology.dal.mysql.consultation.CrisisInterventionMapper;
import com.lvye.mindtrip.module.psychology.dal.mysql.consultation.CrisisEventProcessMapper;
...
public class AssessmentTaskServiceImpl implements AssessmentTaskService {
    ...
    @Resource
    private com.lvye.mindtrip.module.psychology.dal.mysql.assessment.AssessmentScenarioMapper assessmentScenarioMapper;
    ...
    public AssessmentTaskDO getAssessmentTask(String taskNo) {
        ...
        if (assessmentTaskDO.getScenarioId() != null) {
            try {
                com.lvye.mindtrip.module.psychology.dal.dataobject.assessment.AssessmentScenarioDO scenario =
    ...
    public PageResult<AssessmentTaskVO> getAssessmentTaskPage(AssessmentTaskPageReqVO pageReqVO) {
        ...
        if (page.getRecords() != null && !page.getRecords().isEmpty()) {
            for (AssessmentTaskVO record : page.getRecords()) {
                ...
                var items = new java.util.ArrayList<com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.SlotAssignmentVO>();
                var rows = taskQuestionnaireMapper.selectListByTaskNo(record.getTaskNo(), TenantContextHolder.getTenantId());
                if (rows != null) {
                    for (var r : rows) {
                        if (r.getSlotKey() != null) {
                            var item = new com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.SlotAssignmentVO();
    ...
    private List<com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.AssessmentDeptNodeVO> buildDeptTreeStatistics(String taskNo) {
        // 1) 聚合查询：按年级/班级分组，得到 total/completed
        List<com.lvye.mindtrip.module.psychology.dal.dataobject.assessment.AssessmentDeptAggregationRow> rows = userTaskMapper.selectAggregationByDept(taskNo);
        ...
        // 3) 批量查询部门名称与排序
        List<com.lvye.mindtrip.module.system.dal.dataobject.dept.DeptDO> deptList = deptService.getDeptList(deptIds);
        Map<Long, com.lvye.mindtrip.module.system.dal.dataobject.dept.DeptDO> deptMap = deptList.stream()
                .collect(Collectors.toMap(com.lvye.mindtrip.module.system.dal.dataobject.dept.DeptDO::getId, d -> d));
        // 4) 先按年级分组，再生成班级 children
        Map<Long, List<com.lvye.mindtrip.module.psychology.dal.dataobject.assessment.AssessmentDeptAggregationRow>> byGrade = rows.stream()
                .collect(Collectors.groupingBy(r -> r.getGradeDeptId() == null ? -1L : r.getGradeDeptId()));
        List<com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.AssessmentDeptNodeVO> gradeNodes = new ArrayList<>();
        for (Map.Entry<Long, List<com.lvye.mindtrip.module.psychology.dal.dataobject.assessment.AssessmentDeptAggregationRow>> e : byGrade.entrySet()) {
            Long gradeId = e.getKey();
            List<com.lvye.mindtrip.module.psychology.dal.dataobject.assessment.AssessmentDeptAggregationRow> classRows = e.getValue();
            // 构建班级节点
            List<com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.AssessmentDeptNodeVO> classNodes = new ArrayList<>();
            for (var r : classRows) {
                Long classId = r.getClassDeptId() == null ? -1L : r.getClassDeptId();
                com.lvye.mindtrip.module.system.dal.dataobject.dept.DeptDO classDept = classId > 0 ? deptMap.get(classId) : null;
                ...
                com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.AssessmentDeptNodeVO node = new com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.AssessmentDeptNodeVO();
            ...
            classNodes.sort((a, b) -> {
                ...
                com.lvye.mindtrip.module.system.dal.dataobject.dept.DeptDO ad = (aId != null && aId > 0) ? deptMap.get(aId) : null;
                com.lvye.mindtrip.module.system.dal.dataobject.dept.DeptDO bd = (bId != null && bId > 0) ? deptMap.get(bId) : null;
            ...
            java.math.BigDecimal gRate = gTotal == 0 ? java.math.BigDecimal.ZERO
                    ...
                    .multiply(new java.math.BigDecimal("100"));
            com.lvye.mindtrip.module.system.dal.dataobject.dept.DeptDO gradeDept = gradeId != null && gradeId > 0 ? deptMap.get(gradeId) : null;
            String gradeName = gradeDept != null ? gradeDept.getName() : "未知年级";
            com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.AssessmentDeptNodeVO gNode = new com.lvye.mindtrip.module.psychology.controller.admin.assessment.vo.AssessmentDeptNodeVO();
