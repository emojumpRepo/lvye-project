...
public interface StudentInterventionMapper extends BaseMapperX<StudentProfileDO> {
    ...
    @Select("<script>" +
            ...
            "GROUP BY sp.id, sp.name, sp.student_no, sp.sex, sp.risk_level, sp.graduation_status, d1.name " +
    ...
    @Select("<script>" +
            ...
            "  AND EXISTS (SELECT 1 FROM lvye_student_counselor sc WHERE sc.student_profile_id = sp.id AND sc.counselor_user_id = #{counselorUserId} AND sc.deleted = 0) " +
            "</if>" +
            "</script>")
