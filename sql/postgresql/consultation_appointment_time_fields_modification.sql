...
SET appointment_start_time = appointment_time,
    appointment_end_time = appointment_time + INTERVAL '1 minute' * COALESCE(duration_minutes, 60)
...
-- 4. 设置新字段为非空（在数据迁移完成后）
ALTER TABLE lvye_consultation_appointment 
ALTER COLUMN appointment_start_time SET NOT NULL,
