...
public class ProjectReactor {
    ...
    private static final String GROUP_ID = "com.lvye.boot";
    private static final String ARTIFACT_ID = "mindtrip";
    private static final String PACKAGE_NAME = "com.lvye.mindtrip";
    private static final String TITLE = "心之旅AI";
    ...
    public static void main(String[] args) {
        ...
        // ========== 配置，需要你手动修改 ==========
        String groupIdNew = "com.lvye.boot";
        String artifactIdNew = "mindtrip";
        String packageNameNew = "com.lvye.mindtrip";
        String titleNew = "心之旅AI";
    ...
    private static Collection<File> listFiles(String projectBaseDir) {
        ...
        try {
            Path startPath = Paths.get(projectBaseDir);
            Files.walkFileTree(startPath, new SimpleFileVisitor<Path>() {
                @Override
                public FileVisitResult preVisitDirectory(Path dir, BasicFileAttributes attrs) {
                    String path = dir.toString();
                    // 在进入目录之前就判断是否需要跳过
                    if (path.contains(separator + "target")
                            || path.contains(separator + "node_modules")
                            || path.contains(separator + "mindtrip-ui")
                            || path.contains(separator + ".idea")
                            || path.contains(separator + ".git")
                            || path.contains(separator + "dist")) {
                ...
                public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
                    ...
                    // 过滤掉不需要的文件
                    if (!path.contains(".iml") && !path.contains(".html.gz")) {
                ...
                public FileVisitResult visitFileFailed(Path file, IOException exc) {
                    // 遇到访问失败的文件（如符号链接错误）时，记录日志并继续
                    log.warn("[listFiles][访问文件失败，跳过：{}]", file, exc);
                    return FileVisitResult.CONTINUE;
                }
            });
        } catch (IOException e) {
            log.error("[listFiles][遍历文件失败]", e);
